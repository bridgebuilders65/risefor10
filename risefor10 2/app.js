...
document.addEventListener("click",e=>{
  if(panel.classList.contains("open")&&!panel.contains(e.target)&&e.target.id!=="worldmap"&&!e.target.closest("[data-active]")){
    closePanel();
  }
});
...
function openPanel(country){
  ...
  document.getElementById("pMeta").textContent=allCount+" physician"+(allCount!==1?"s":"")+(cities.length?" · "+cities.join(" · "):"");
...
// Pan + pinch (no scroll-wheel zoom)
    ...
    events:{
      ...
      onStateChange:function(e){
        if(e.data===1&&!ytReady){
          ...
          var th=document.getElementById("videoThumb");if(th)th.style.display="none";
...
async function pollParticipation(){
  try{
    ...
    Object.keys(liveByCountry).forEach(country=>{
      ...
      g.setAttribute("class","live-pin-group");g.style.cursor="pointer";
      const ring=document.createElementNS("http://www.w3.org/2000/svg","circle");
      ring.setAttribute("cx",x);ring.setAttribute("cy",y-14);ring.setAttribute("r","7");ring.setAttribute("class","live-pin-ring");
      const dot=document.createElementNS("http://www.w3.org/2000/svg","circle");
      dot.setAttribute("cx",x);dot.setAttribute("cy",y-14);dot.setAttribute("r","4");dot.setAttribute("class","live-pin");
